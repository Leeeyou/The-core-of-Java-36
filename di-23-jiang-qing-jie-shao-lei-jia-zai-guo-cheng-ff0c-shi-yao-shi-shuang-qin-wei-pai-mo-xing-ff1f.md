...
再来谈谈双亲委派模型，简单说就是**当类加载器（Class-Loader）试图加载某个类型的时候**，**除非父加载器找不到相应类型**，**否则尽量将这个任务代理给当前加载器的父加载器去做**。使用委派模型的目的是**避免重复加载 Java 类型**。

> 这里讲的过于简单，可以参考[《Java虚拟机原理图解》5. JVM类加载器机制与类加载过程](https://blog.csdn.net/luanlouis/article/details/50529868)
...
这能让我们更清楚，**普通原始类型静态变量和引用类型（即使是常量），是需要额外调用 putstatic 等 JVM 指令的，这些是在显式初始化阶段执行，而不是准备阶段调用**；而**原始类型常量，则不需要这样的步骤**（注意上面 1000是个常量，没有出现再初始化逻辑阶段）。
...
android中通常会根据jar包冲突信息来删掉重复包，或者可以排除掉重复的包内容。
